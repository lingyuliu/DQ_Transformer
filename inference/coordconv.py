...
class AddCoords(nn.Module):
    def __init__(self, rank, with_r=False, use_cuda="cpu"):
    ...
    def forward(self, input_tensor):
        ...
        if self.rank == 1:
            ...
            if torch.cuda.is_available and self.use_cuda:
                input_tensor = input_tensor.to(self.use_cuda)
                xx_channel = xx_channel.to(self.use_cuda)
            ...
            if torch.cuda.is_available and self.use_cuda:
                input_tensor = input_tensor.to(self.use_cuda)
                xx_channel = xx_channel.to(self.use_cuda)
                yy_channel = yy_channel.to(self.use_cuda)
            ...
            if torch.cuda.is_available and self.use_cuda:
                ...
                xx_channel = xx_channel.to(self.use_cuda)
                yy_channel = yy_channel.to(self.use_cuda)
                zz_channel = zz_channel.to(self.use_cuda)
...
class CoordConv1d(conv.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, with_r=False, use_cuda="cpu"):
